...
export default async function NewAnime({ searchParams }: { searchParams:  Promise<{ page: string, genre: string }> }) {

  
  const posts = await getSeasonNowAnime();
      ...
      <div className="flex flex-wrap items-center justify-center w-full px-10 py-5">
        {genre.slice(0, 10).map((genre: Genre) => (
          <Badge variant={"secondary"} key={genre.mal_id} className="m-2  text-sm font-sans">
            <Link  href={`/top-anime/?genre=${genre.name}`} prefetch>{genre.name}</Link>
          </Badge>
        ))}
